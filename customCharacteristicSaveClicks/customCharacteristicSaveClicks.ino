...
void setup(void)
{
  ...
  /* Add timestamp to heart rate transmission */
 
  Serial.println(F("Adding the timestamp characteristic (UUID = 0x2A40): "));
  success = ble.sendCommandWithIntReply(F("AT+GATTADDCHAR=UUID=0x2A40, PROPERTIES=0x10, MIN_LEN=2, MAX_LEN=3, VALUE=0"), &hrmTimestampCharId);
    if (! success) {
...
void loop(void)
{
  ...
  ble.print( F("AT+GATTCHAR=") );
  ble.print( hrmTimestampCharId );
  ble.print( F(",00-") );
  ble.println(timestamp, HEX);
